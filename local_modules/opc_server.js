...
async function opc(endpointUrl, nodes, idioma) {
    try { 
        ...
        client.on("backoff", (retry, delay) => {
            ...
            io.sockets.emit('estadoOPCUA',  estadoOPCUA);
        ...
        io.on('connection', function (socket) {
            ...
            io.sockets.emit("idioma", idioma);

            io.sockets.emit("modo", modo);

            io.sockets.emit("mapa", mapa);

            io.sockets.emit("opcua", endpointUrl);
            ...
            socket.on('idioma', (data) => {
                idioma = data;
                console.log('Cambio de idioma solicitado');
                console.log(idioma);
                io.sockets.emit("idioma", idioma);
            })

            socket.on('login', (data) => {
                console.log('Cambio de modo solicitado');
                console.log(data.nombre);
                console.log(data.contraseña);
                if(data.nombre == "inigoDP" && data.contraseña == "ps5zk4jjZUU"){
            ...
            socket.on('cambioModo', (data) => {
                modo = data;
                io.sockets.emit('modo', modo);
            })

            socket.on('cambioMapa', (data) => {
                mapa = data;
                io.sockets.emit('mapa', mapa);
            })
